...
const ContentBannerDisplay: React.FC<{ banner: ContentBanner }> = ({ banner }) => {
    ...
    return (
        <div className="my-8" style={{ width: banner.width, height: banner.height, margin: '32px auto' }}>
            <Wrapper {...props}>
                {bannerContent}
            </Wrapper>
        </div>
...
const LoginPrompt: React.FC<{ onNavigateToLogin: () => void; contentType: string }> = ({ onNavigateToLogin, contentType }) => (
    <div className="mt-10 p-6 rounded-2xl text-center bg-amber-50 border border-amber-200 dark:bg-amber-900/30 dark:border-amber-800">
        <h3 className="text-xl font-bold text-amber-800 dark:text-amber-200">Acesso Restrito</h3>
        <p className="text-amber-700 dark:text-amber-300 mt-2 mb-4">Você precisa fazer login para acessar {contentType}.</p>
        <button
        ...
        switch (category.type) {
            ...
            case 'products':
                ...
                if (contentItems.length === 0) {
                     return (
                         <div key={category.id} className="mt-10 p-6 rounded-2xl text-center dark:!bg-gray-900" style={{ backgroundColor: siteConfig.storeConfig.themeBgColor }}>
    ...
    return (
        <div className="p-4 sm:p-8 fade-in">
            ...
            <div className="text-center mb-8">
                {logoUrl ? (
                    <img src={logoUrl} alt="Site Logo" className="max-w-xs max-h-24 mx-auto mb-4" />
                ) : (
                    <h1 className="text-3xl md:text-4xl font-bold mb-2 dark:text-white" style={{ color: siteConfig.themeConfig.primaryColor }}>{siteConfig.siteTitle}</h1>
                )}
                {siteConfig.showGuestNameInHeader && !currentUser && initialPlayerName && (
                    <p className="text-lg font-medium mt-1 dark:text-gray-300" style={{ color: siteConfig.themeConfig.secondaryTextColor }}>
                        Olá, <span className="font-bold">{initialPlayerName}</span>!
                    </p>
                )}
                <p style={{ color: siteConfig.themeConfig.secondaryTextColor }} className="dark:text-gray-400">{siteConfig.siteSubtitle}</p>
            </div>
