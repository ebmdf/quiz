...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess, onClose, initialView, emailToReset: emailFromProp }) => {
    ...
    const [email, setEmail] = useState(''); // Empty by default for manual entry
                    ...
                    <form onSubmit={handleRecover} className="space-y-6">
                        {success ? (
                            <div className="space-y-4 text-center">
                                <div className="text-green-700 dark:text-green-200 bg-green-100 dark:bg-green-900/50 px-4 py-3 rounded text-sm" role="alert">{success}</div>
                                {siteConfig.adminCredentials.recoveryToken && recoveryLink && (
                                     <div>
                                        <label className="text-sm font-medium text-gray-700 dark:text-gray-300 block text-left mb-1">Link de Recuperação:</label>
                                        <input type="text" readOnly value={recoveryLink} className="w-full bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 p-2 rounded text-xs text-gray-800 dark:text-gray-200" onFocus={(e) => e.target.select()}/>
                                    </div>
                                )}
                                 <div className="text-center text-sm"><button type="button" onClick={() => { setView('login'); setSuccess(''); setError(''); }} className="font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500">Voltar para o Login</button></div>
                            </div>
                        ) : (
                            <>
                                <p className="text-sm text-gray-600 dark:text-gray-300 text-center">Insira o e-mail de administrador para iniciar a recuperação.</p>
                                <div>
                                    <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
                                    <input id="email" type="email" required value={email} onChange={e => setEmail(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded shadow-sm" />
                                </div>
                                ...
                                <div className="text-center text-sm"><button type="button" onClick={() => setView('login')} className="font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500">Voltar para o Login</button></div>
                            </>
                        )}
                    </form>
                );
            ...
            case 'reset':
                return (
                    <form onSubmit={handleReset} className="space-y-6">
                         <p className="text-sm text-gray-600 dark:text-gray-300 text-center">Crie uma nova senha para <strong className="text-gray-800 dark:text-white">{emailToReset}</strong>.</p>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nova Senha</label>
                            <input type="password" required value={newPassword} onChange={e => setNewPassword(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded shadow-sm" />
                        </div>
                        <PasswordCriteria password={newPassword} />
                         <div>
                            ...
                            <input type="password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded shadow-sm" />
                        </div>
                        {error && <p className="text-red-500 text-sm text-center">{error}</p>}
                        <div>
            ...
            default:
                return (
                     <form onSubmit={handleLogin} className="space-y-6">
                        {success && <div className="bg-green-100 dark:bg-green-900/50 border border-green-400 dark:border-green-700 text-green-700 dark:text-green-200 px-4 py-3 rounded text-center" role="alert">{success}</div>}
                        <div>
                            <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
                            <input id="email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded shadow-sm" />
                        </div>
                        <div>
                            <label htmlFor="password"className="block text-sm font-medium text-gray-700 dark:text-gray-300">Senha</label>
                            <input id="password" type="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded shadow-sm" />
                        </div>
                        {/* Forgot Password link removed as requested */}
                        {error && <div className="bg-red-100 dark:bg-red-900/50 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-200 px-4 py-3 rounded" role="alert">{error}</div>}
                        <div>
    ...
    return (
        <div className="admin-overlay" onClick={onClose}>
            <div 
                className="admin-panel bg-gray-100 dark:bg-gray-900 rounded-lg shadow-2xl flex flex-col overflow-hidden max-w-md !h-auto border dark:border-gray-700" 
            ...
            >
                <header className="flex justify-between items-center p-4 bg-white dark:bg-gray-800 border-b dark:border-gray-700 flex-shrink-0">
                    <h2 className="text-xl font-bold text-gray-800 dark:text-white">{titles[view]}</h2>
